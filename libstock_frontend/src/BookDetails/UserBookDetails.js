  ...
  useEffect(() => {
    ...
    const fetchBookDetails = async () => {
      try {
        ...
        setAuthor(authorRes.data.length > 0 
          ? authorRes.data.map((a) => `${a.firstName} ${a.lastName}`).join(", ") 
          : "Unknown Author"
        );
        setGenres(genreRes.data.length > 0 
          ? genreRes.data.map((g) => g.name).join(", ") 
          : "Unknown Genre"
        );

        await Promise.all([fetchUserFavorites(), fetchUserWishlist(), fetchUserCheckouts()]);
  ...
  const handleFavoriteToggle = async () => {
    ...
    try {
      if (favoriteBooks.has(bookId)) {
        await axios.delete(`http://localhost:8080/favorite/delete`, { params: { userId, bookId } });
  ...
  const handleWishlistToggle = async () => {
    ...
    try {
      if (wishlist.has(bookId)) {
        await axios.delete(`http://localhost:8080/wishlist/delete`, { params: { userId, bookId } });
        setWishlist((prev) => {
    ...
    <div className="UserBook-details-container">
      ...
      <span className="favorite-icon" onClick={handleFavoriteToggle}>
        {favoriteBooks.has(book.id) ? (
          <FavoriteIcon style={{ cursor: "pointer", color: "red", fontSize: "24px", marginLeft: "580px" }} />
        ) : (
